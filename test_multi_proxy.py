...
class Evaluator:
    ...
    def __init__(self, dset, model):
        ...
        #这段代码的作用是将self.test_pair_dict中的每个元素作为键创建一个新的字典，并将所有键的初始值设置为0。
        ...
        #对字典赋值，key为（属性id，类别id），value为[属性-类别对的id，0，0]
        # open world
        if dset.open_world:
    ...
    def generate_predictions(self, scores, obj_truth, bias=0.0, topk=1):  # (Batch, #pairs)
        ...
        def get_pred_from_scores(_scores, topk):
            ...
            pair_pred = pair_pred.contiguous().view(-1) #变成一维连续数组
            attr_pred, obj_pred = self.pairs[pair_pred][:, 0].view(
                -1, topk
            ), self.pairs[pair_pred][:, 1].view(-1, topk) #从pair_pred中取出attr和obj
...
def predict_logits(model, dataset, config):
    ...
    attr_dataset=dataset.attrs
    obj_dataset=dataset.objs
    ...
    with torch.no_grad():
        for idx, data in tqdm(
            ...
            # batch_pair=data[3].cuda()
            # batch_attr= data[1].cuda()
            # batch_obj =data[2].cuda()
            # batch_pair=data[3].cuda()
            
            predict= model(data,pairs)
           

            loss, loss_logit_com_proxy, loss_logit_attr_proxy, loss_logit_obj_proxy = loss_calu(predict, data, config)[:4]
            loss_logit_com_text, loss_logit_attr_text, loss_logit_obj_text, loss_com= loss_calu(predict, data, config)[4:]
            # loss += loss_calu(predict, data, config)
            total_logits,A, O = predict
            logits_com_proxy,logits_attr_proxy,logits_obj_proxy,logits_com_text,logits_attr_text,logits_obj_text=total_logits
            comp_logits=config.alpha*logits_com_proxy+config.beta*logits_com_text
            attr_logits=config.alpha*logits_attr_proxy+config.beta*logits_attr_text
            obj_logits=config.alpha*logits_obj_proxy+config.beta*logits_obj_text
            attr_pred = F.softmax(attr_logits, dim=-1)
            obj_pred = F.softmax(obj_logits, dim=-1)
            for i_comp in range(comp_logits.shape[-1]):
                weighted_attr_pred=attr_pred[:, pairs[i_comp][0]]
                weighted_obj_pred=obj_pred[:, pairs[i_comp][1]]
                comp_logits[:, i_comp] = comp_logits[:, i_comp] + weighted_attr_pred+weighted_obj_pred
            logits=comp_logits
    ...
    return all_logits, all_attr_gt, all_obj_gt, all_pair_gt, loss / len(dataloader),out_dict
...
if __name__ == "__main__":
    ...
    if config.open_world and config.threshold is None:
        ...
        feasibility_path = os.path.join(
            DIR_PATH, f'save/feasibility_{config.dataset}.pt')
        ...
        val_stats = None
        with torch.no_grad():
            all_logits, all_attr_gt, all_obj_gt, all_pair_gt, loss_avg,out_dict = predict_logits(
        ...
        with torch.no_grad():
            ...
            # pd.DataFrame(out_dict).to_csv(os.path.join(config.save_path, f"result.csv"))
    ...
    with torch.no_grad():
        ...
        # pd.DataFrame(out_dict).to_csv(os.path.join(config.save_path, f"{config.dataset}_result.csv"))
