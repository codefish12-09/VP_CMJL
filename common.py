...
class CrossResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        output,weights=self.attention(self.ln_x(x), self.ln_y(y))
        x = x + output
        x = x + self.mlp(self.ln_2(x))
        return x,weights
    
class MultiLayerCrossResidualAttention(nn.Module):
    def __init__(self, d_model: int, n_head: int, num_layers: int, attn_mask: torch.Tensor = None):
        super().__init__()

        # 创建多个 CrossResidualAttentionBlock 实例
        self.layers = nn.ModuleList([
            CrossResidualAttentionBlock(d_model, n_head, attn_mask) for _ in range(num_layers)
        ])

    def forward(self, x: torch.Tensor, y: torch.Tensor):
        weights_list = []  # 用于存储每层的权重
        for layer in self.layers:
            x, weights = layer(x, y)
            # weights_list.append(weights)  # 收集每层的权重

        return x, weights # 返回最终的 x 和所有层的权重
